...
//////////////////////////////
  ...
  // Initialize markup percentages with default values (20%)
  // useEffect(() => {
  //   const initialMarkups = {};
  //   materials.forEach(material => {
  //     initialMarkups[material.id] = 0.00; // Default 20% markup
  //   });
  //   setMarkupPercentages(initialMarkups);
  // }, [materials]);
const [markupsInitialized, setMarkupsInitialized] = useState(false);
  ...
  // Add this function to your AddCostEstimationPage component

/*const exportDataForBackend = () => {
  // Extract quotation number from the page (you might need to make this dynamic)
  const quotationNumber = "PEQ/25/03/001/2"; // This should come from your state or props
  ...
  // Process each material as an "item" in the backend format
  materials.forEach(material => {
    const materialId = material.id;
    const materialName = material.name;
    const materialQuantity = itemQuantities[materialId] || 0;

    // Create item materials array by processing all sections and their items
    const itemMaterials = [];
    
    // Process each section to collect item materials
    sections.forEach(section => {
      section.items.forEach(item => {
        // Get the quantity for this material from this item
        const materialIndex = materials.findIndex(m => m.id === materialId);
        const quantity = item.quantities[materialIndex] || 0;
        
        // Only add if quantity > 0
        if (quantity > 0) {
          itemMaterials.push({
            materialId: item.id, // Using item.id as materialId
            materialQuantity: quantity,
            unitPrice: item.unitPrice || item.marketPrice || 0,
    ...
    // Create the item object with markup percentages
    const itemData = {
      itemName: materialName,
      itemQuantity: materialQuantity,
      SwitchGearCompMarkup: markupPercentages[materialId] || 20.00,
      ControlAccessoryMarkup: markupPercentages[materialId] || 20.00,
      BusBarMarkup: markupPercentages[materialId] || 20.00,
      WiringMarkup: markupPercentages[materialId] || 20.00,
      OtherAccessoryMarkup: markupPercentages[materialId] || 20.00,
      ElectricalLabourMarkup: markupPercentages[materialId] || 20.00,
      TransportMarkup: markupPercentages[materialId] || 20.00,
      EnclosureMarkup: markupPercentages[materialId] || 20.00,
      itemMaterials: itemMaterials
    };

    // Only add items that have materials or quantity > 0
    if (itemMaterials.length > 0 || materialQuantity > 0) {
  ...
  const renderPageHeader = () => (
    <div className="mb-4">
      ...
      <p className="text-gray-600 text-sm">
        Quotation Number: {quotationNumber || '[New Estimation]'}
      </p>
    </div>
  );
